...
export default function FocusAnalyticsPanel() {
  ...
  return (
    <div className="overflow-hidden rounded-3xl border-2 border-white/70 bg-white/80 shadow-md backdrop-blur">
      <div className="flex items-center justify-between gap-3 bg-gradient-to-r from-sky-blue-300 to-mint-300 px-6 py-3">
        <h2 className="font-title text-lg text-white">📊 집중도 분석</h2>
        <div className="flex shrink-0 gap-1 rounded-full bg-white/30 p-0.5">
          <button
            onClick={() => setPeriod("weekly")}
            className={`rounded-full px-3 py-1 text-xs font-bold transition ${
              period === "weekly" ? "bg-white text-[#2b6f8f]" : "text-white/90"
            }`}
          >
            주간
          </button>
          <button
            ...
            className={`rounded-full px-3 py-1 text-xs font-bold transition ${
              period === "monthly" ? "bg-white text-[#2b6f8f]" : "text-white/90"
            }`}
          >
            월간
          </button>
          ...
          <>
            ...
            <div className="mb-5 grid grid-cols-3 gap-2">
              <div className="rounded-2xl bg-sky-blue-50 px-2 py-3 text-center">
                <p className="text-[10px] font-bold uppercase tracking-wide text-[#8fb0c4]">
                  {period === "weekly" ? "주간" : "월간"} 평균
                </p>
                <p className="mt-1 text-sm font-bold text-[#2b6f8f]">{formatDuration(data.averageSeconds)}</p>
              </div>
              <div className="rounded-2xl bg-mint-50 px-2 py-3 text-center">
                <p className="text-[10px] font-bold uppercase tracking-wide text-[#8fb8a6]">목표 달성률</p>
                <p className="mt-1 text-sm font-bold text-[#3a6e58]">
                  {data.goalAchievementRate}%
                  <span className="ml-1 text-[10px] font-normal text-[#8fb8a6]">
                    ({data.goalMetDays}/{data.periodDays}일)
                  </span>
                </p>
              </div>
              <div className="rounded-2xl bg-angel-pink-50 px-2 py-3 text-center">
                <p className="text-[10px] font-bold uppercase tracking-wide text-[#cdb8c4]">연속 학습</p>
                <p className="mt-1 text-sm font-bold text-[#ff6fa5]">🔥 {data.streakDays}일</p>
              </div>
            </div>

            {/* 목표 설정 */}
            <div className="mb-4 flex items-center justify-between rounded-2xl bg-[#fdf6f0] px-4 py-2.5">
              {editingGoal ? (
                <div className="flex w-full items-center gap-2">
                  <span className="text-xs font-bold text-[#a8889a]">하루 목표</span>
                  <input
                    autoFocus
                    type="number"
                    min={1}
                  ...
                  />
                  <span className="text-xs font-bold text-[#a8889a]">분</span>
                </div>
              ) : (
                <>
                  <span className="text-xs font-bold text-[#a8889a]">🎯 하루 목표 {data.goalMinutes}분</span>
                  <button onClick={startEditGoal} className="text-xs font-bold text-[#ff6fa5] underline">
                    수정
                  </button>
                </>
              )}
            ...
            <p className="mb-1.5 text-xs font-bold uppercase tracking-wide text-[#cdb8c4]">
              ...
              {data.daily.map((d, i) => {
                ...
                return (
                  <div key={d.date} className="group flex flex-1 flex-col items-center gap-1">
                    ...
                    {showLabel && <span className="text-[9px] text-[#cdb8c4]">{formatDay(d.date)}</span>}
                  </div>
                );
              })}
            </div>
