...
>(({ config, totalSeconds, audioPresetName, completedTitle, completedDate }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={`relative flex h-[480px] w-[360px] flex-col justify-between rounded-2xl p-8 text-white ${background.className} ${font.className}`}
    >
      {config.sticker && <span className="absolute right-6 top-6 text-3xl">{config.sticker}</span>}

      <div>
        <p className="text-sm uppercase tracking-widest text-white/60">DeskLog</p>
        <h2 className="mt-2 text-2xl font-semibold">{completedTitle ?? "나의 독서 기록"}</h2>
      </div>

      <div className="space-y-3">
        <div>
          <p className="text-xs text-white/60">총 독서 시간</p>
          <p className="text-3xl font-bold">{formatHours(totalSeconds)}시간</p>
        </div>
        {audioPresetName && (
          <div>
            <p className="text-xs text-white/60">주요 사운드</p>
            <p className="text-lg">{audioPresetName}</p>
          </div>
        )}
        {completedDate && (
          <div>
            <p className="text-xs text-white/60">완독 날짜</p>
